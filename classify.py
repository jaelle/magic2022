...
# NOTE: THIS IS THE MOSTG IMPORTANT PART OF THE CODE!!
def run(model: str, max_results: int, num_threads: int, enable_edgetpu: bool,
  ...
  while cap.isOpened():
    ...
    cv2.putText(image, str(categories[0].label), text_location, cv2.FONT_HERSHEY_PLAIN,
                  _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)
    if not button1.value:
        os.system('echo "' + categories[0].label + '"  | festival --tts')
